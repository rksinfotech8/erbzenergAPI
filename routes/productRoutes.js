...
router.route('/powder/:id').get((request, response) => {
  const { powderId } = request.params;
  product.getPowderId(powderId)
      .then(result => {
...
router.route('/tablet/:id').get((request, response) => {
  const { powderId } = request.params;
  product.getTabletId(powderId)
      .then(result => {
          if (result && result.length > 0) {
              response.json(result[0]);
          } else {
...
router.route('/capsule/:id').get((request, response) => {
  const { powderId } = request.params;
  product.getCapsuleId(powderId)
      .then(result => {
          if (result && result.length > 0) {
              response.json(result[0]);
          } else {
